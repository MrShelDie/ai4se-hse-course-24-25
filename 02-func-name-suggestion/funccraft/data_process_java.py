...
def remove_comments_node(node: Node, code: str, offset: int) -> tuple[str, int]:
    """
    Removes comments from the given code based on the current node.
...
def process_function_java(code: str) -> dict[str, str]:
    """
    Process a Java function by extracting its name, replacing its name with a special token, and removing comments.

    Args:
        code (str): The original code of the Java function.
