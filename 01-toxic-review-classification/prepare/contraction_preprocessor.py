...
def expand_contraction(text: str) -> str:
    """
    Expand contractions in a given text to their full forms.

    This function replaces various forms of apostrophes with a standard 
    apostrophe and expands contractions in a string by using a predefined 
    mapping of contractions to their expanded forms. Words that are not in 
    the contraction mapping remain unchanged.

    Args:
        text (str): The input text containing contractions.

    Returns:
        str: The text with contractions expanded to their full forms.
    """
    text = text.replace('’', "'")
    text = text.replace('‘', "'")
    text = text.replace('´', "'")
